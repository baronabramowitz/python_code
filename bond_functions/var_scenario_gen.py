...
def strips_data_generation_for_var(bond_portfolio_currency):
    """Generate a VaR scenario of different yield curves.

    Retrieve STRIPS data from US Treasuries or UK Gilts and
...
def var_strips_data_generation(data_start_date, var_days, sample_fraction, currency):
    ...
    rand_sample = ustreasury_yield_delta_modern.sample(
        frac=sample_fraction, replace=False)
    ...
    spl_list = [InterpolatedUnivariateSpline(
        xrange, row[1:]) for row in rand_sample.itertuples()]
    years_for_payments = [days / 365 for days in mat_dates]
    yield_change_list = [
        [spl(years) for years in years_for_payments] for spl in spl_list]
    ...
    yield_scenario_set = [[y1 + y2 for y1, y2 in zip(
        yield_delta_set, base_yield_points)] for yield_delta_set in yield_change_list]
    final_spl_set = [InterpolatedUnivariateSpline(
        mat_dates, y_set)for y_set in yield_scenario_set]
    print('VaR SDG End   ', datetime.now())
    return final_spl_set

if __name__ == "__main__":
    var_strips_data_generation('1985-12-15', 10, .1, 'USD')
